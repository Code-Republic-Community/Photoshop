...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.add_menus()
        self.keys = list(self.menu__and_actions.keys())
        self.connect_actions()


    def connect_actions(self):

        self.menu__and_actions[self.keys[0]][5].triggered.connect(self.close)


    def add_menus(self):
        menubar = self.menuBar()

        for name,actions in self.menu__and_actions.items():
